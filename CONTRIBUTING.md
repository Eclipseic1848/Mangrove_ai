...
根据变更风险选择最小充分门禁，并在 PR 中记录真实结果：

```powershell
py -3.13 -X utf8 -m pytest <相关测试>
Set-Location frontend
npm run build
npm run test:e2e
```

涉及启动、端口、Docker 或 Runtime 的变更，还应实际启动服务、检查
`http://127.0.0.1:8088/api/health`，并在停止后确认端口、容器和任务级网络无残留。自动化测试
通过不等于用户验收或生产资格。
