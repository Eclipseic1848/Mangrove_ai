...
判断规则：
- 明确要求定时执行但时间或周期缺失时先追问，不能用默认时间；否定、假设、示例和引用不构成定时创建授权。
...
数据与目标主题或指定平台不符、相关样本为零、或用户明确要求的数量未满足时，
必须 passed=false、score ≤ 40。即使报告如实说明了失败且结构完整，也不能判定任务通过。
采集条数上限本身不等于用户明确要求的最低数量，需结合任务目标判断。
