"""连接来源 API 只装配现有公开 Source DTO，不接受凭据或回调。"""
...
class AcquireIn(BaseModel):
    ...
    expected_connection_version: str | None = Field(
        default=None,
        pattern="^[0-9a-f]{64}$",
    )
    ...
    *,
    ...
    def resolve(payload: SourceIn, user=Depends(get_execution_user)):
        try:
            frozen = freeze(
                user["user_id"],
                resolve_source(user["user_id"], payload, "resolve"),
            )
            scope = ConnectorRequest(frozen, "连接来源核验").allowed_scope()
            return {
                key: value
                for key, value in scope.items()
                if key != "artifact_namespace"
            }
        except PermissionError:
            raise HTTPException(404, "连接来源不存在或无权读取") from None
        except ValueError:
            raise HTTPException(422, "连接来源配置无效") from None
    ...
    async def acquire(
        ...
        try:
            def load():
                return resolve_source(owner, payload.source, idempotency_key)

            row = load()
            frozen = freeze(owner, row)
            if (
                payload.expected_connection_version
                and payload.expected_connection_version != frozen.version
            ):
            ...
            active_repository = repository or SourceAcquisitionRepository(
            ...
            return await acquire_connector(
                ...
                owner=owner,
                key=idempotency_key,
                request=ConnectorRequest(frozen, payload.purpose),
                load_current=load,
                connector=connector_factory(row),
                artifact_store=artifact_store or ArtifactStore(),
                resume_checkpoint=payload.resume_checkpoint,
                pause_after_batches=batch_budget,
            )
        except PermissionError:
            raise HTTPException(404, "连接来源不存在或无权读取") from None
        except ValueError:
...
def resolve_registered_source(owner, source, key):
    """沿用公开 SourceSpec 与 Owner 连接登记，不创建第二套连接系统。"""
    import hashlib
    import json
    ...
    # 原件命名空间绑定 Owner 与幂等请求，不能由客户端选择或跨请求复用。
    spec.options["task_id"] = "connector-" + hashlib.sha256(
        (owner + "\0" + key).encode()
    ).hexdigest()
    return {
        "owner": owner,
...
def connector_for(row):
    """固定内部装配；公开请求不能选择 transport 或凭据 resolver。"""
