...
默认提案预算为单个已选本地 Qwen3.8-27B、一次冻结运行、24 次调用、并发 1、每调用最大输出 2048 tokens、序列化输入最多 64 KiB。具体端点只记本地评测清单，不把本机地址写入公开仓库。该输入上限是字节，不假装精确 token 计数；总输出 token 上限为 49152。超预算/超限在发送前停；禁用 SDK/框架隐式重试及自动修复额外调用。超时、断连或结果未知保留证据并停止，未经重新明确预算不自动补跑。费用使用实际可得计量；未知不填零，不估算账单。
