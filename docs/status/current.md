...
>
> last_verified: 2026-09-08（#132 完整来源导出工程实现；合并状态现场核对 GitHub）
>
...
- 当前整改权威为 [#113](https://github.com/Eclipseic1848/Mangrove_ai/issues/113)。
  #131 已由 PR #167 合并关闭；本分支实现 #132，依赖 #129、#126 已关闭。后续按工单依赖继续，不代表整个 P1 完成。
- 所选冻结修订可下载完整资料包：原件、Markdown、JSON/JSONL 与可选 CSV/XLSX；不要求已有正式结果。
  原结果包可附同一来源清单，保留正式交付 QA、Owner、修订及字节摘要校验。
- 网页全文从已保存原件导出，保留来源/最终 URL、读取时间、链接、快照、记录数与解析缺口。
  导出不重采集、不调用模型或 OCR；无法解析、资源未抓取、投影损失及上限须明确标记，原件不删改。
- 异常、断连及存储不足只清理本次临时包；界面支持表格选择、取消和失败重试，任务/修订变化拒绝迟到下载。
- [#132 规格](../plans/2026-09-08-complete-source-export-spec.md)记录范围和门禁；隔离 HTTP、格式与浏览器验证不代表真实业务或生产资格。
  同提交 CI、两轴审查通过后才可合并关闭，实际状态以关联 PR 为准。
