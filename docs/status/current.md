...
>
> last_verified: 2026-09-09（#134 无网址公开搜索工程实现；合并状态现场核对 GitHub）
>
...
- 当前整改权威为 [#113](https://github.com/Eclipseic1848/Mangrove_ai/issues/113)。
  #133 已由 PR #170 合并关闭；本分支实现 #134，依赖已关闭。后续按工单依赖继续，不代表整个 P1 完成。
- 统一工作台可从无附件、无网址的需求进入明确公开搜索；时间、限定域名及读取数量与查询冻结，单网址保持既有范围。
- 固定搜索服务只产生候选链接；逐页受控读取后保存正文快照和原件，报告区分发现、已读、失败、越范围。零结果与安全拒绝不等于全网不存在。
- 搜索报告经显式 webui_0015 迁移单列持久保存；任务、原文资料包与明确刷新继续复用已有流程，不重复读取同一幂等请求。此处没有执行生产库迁移。
- 部分结果保留并披露缺口，硬数量不足仍拒绝启动；有界查询始终不承诺全网完整。取消和撤权在实际发送前复查，拒绝迟到结果并关闭资源。
- [#134 规格](../plans/2026-09-09-public-query-search-spec.md)记录范围与门禁。合成 HTTP 和浏览器证据不等于真实模型质量；本机匿名探针被网络安全预检拒绝、未发 HTTP，不授真实搜索可用或生产资格。
