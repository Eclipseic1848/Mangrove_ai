...
| AC-05 隔离能力获取 | 带备份生产迁移与工程验收通过 | 等待用户最终验收结论；不得与业务来源同时联网 |
...
- G2 剩余门：Word/Excel 连续 3/3 已通过；AC-05 带备份生产迁移、恢复副本状态机与真实
  Docker 探针已通过。最终用户验收结论尚未给出，因此完整 PG-05 仍未完成。
...
2026-08-22，G2 Office 与 AC-05 工程门通过，最终用户验收结论待确认：
...
- 恢复副本上的幂等、请求身份、Owner 权限、取消和终态 CAS 全部通过；真实 Docker 获取、
  冻结挂载、取消、重试和崩溃恢复通过且残留为 0；8088 重启健康。Issue #38、完整 G2 和
  Phase 4 仍等待用户最终验收结论及后续远端动作。

## 7. 当前优先顺序

1. **#40 G1 本地工程门已达标**：v3 正式结果功能 96.8%、安全 100%，资格 PASS；下一步是
   在用户单独授权后更新/关闭 Issue、推送分支并建 PR。当前本地结果不得冒充远端工单已完成。
2. **G2 工程门通过、G3 未开工**：G2 Office 3/3 与 AC-05 生产迁移/恢复/真实探针均已通过，
   下一门是用户给出 G2 最终验收结论；G3 默认入口切换仍需独立规格、实现与用户授权。
