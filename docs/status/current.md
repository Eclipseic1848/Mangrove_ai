...
- G4 外部 Provider 工程准备已完成但尚未形成生产资格：已实现冻结清单、普通合成 Owner 的
  ...
  不能重复提交，旧失败记录保留；该批修复已提交为 `9ba95985`，相关后端 166 项、前端 23 项
  及生产构建通过。绑定该提交的传输安全矩阵 6/6 通过；隔离 Relay 的真实 DeepSeek 合成运行
  已正确生成单个候选并通过验证，数据库实际记录 8 条 `recorded` Usage，但资格脚本仍按旧的
  “只能有 1 条 Usage”口径误报 `missing`。现已改为汇总多轮 Usage，并在任一轮未知时失败关闭，
  相关后端 169 项通过；该修复尚未提交，DeepSeek 资格重跑与百炼运行均未开始。生产 Vault
  轮换按用户决定不执行，最终 G4 汇总仍不完整。
...
- `explicit_opt_in` 切换前曾核验出缺少“获准用户”资格门，并创建修复工单 #43。当前产品目标
  是“可用后所有用户默认使用”，因此 OptInGrant 方案不再实施；#43 已被后续产品
  决策取代，但远端状态尚未调整。生产继续保持 `admin_gray`，G4 合格与默认切换单独授权均未
  完成前，不得进入 `vnext_default`。
...
2. **G2 已通过、G3 `admin_gray` 恢复验收已通过**：8088 已加载 G3，新活动快照六项门累计
   有效合格，恢复动作、技术烟测和用户确认均完成。产品方向已改为 G4 合格后全用户默认；
   当前没有默认切换授权，生产不得扩大。
3. **G4 已授权准备、G5 未完成**：G4 仅对平台共享 DeepSeek/百炼使用纯合成数据执行，需先
   建立并通过完整安全矩阵；8B Linux/Compose 与目标服务器仍未就绪。
