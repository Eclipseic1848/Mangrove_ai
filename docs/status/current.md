...
>
> last_verified: 2026-09-09（#135 混合来源工程实现；合并状态现场核对 GitHub）
>
...
- 当前整改权威为 [#113](https://github.com/Eclipseic1848/Mangrove_ai/issues/113)。
  #134 已由 PR #171 合并关闭；本分支实现 #135，依赖 #131、#132、#134 已关闭。后续按工单依赖继续，不代表整个 P1 完成。
- 同一输入可连续添加文件与多个独立网页组，按精确身份去重；完整集合与目标、逐组范围冻结到修订，不按名称或正文相同合并出处。
- 新修订可显式移除某类来源，定向网页刷新只替换所选组；运行与原件资料包从选定修订读取，旧版本不随当前选择改变。
- 已完成上传与网页引用可恢复；未完成上传刷新后提示重新选择。响应未知保留原请求，精确幂等回执用于恢复，不能猜测新版本归属。
- webui_0016 仅为修订新增可空来源合同；旧 NULL 回读旧合同，移除最后网页仍保留目标，硬数量与范围缺口不由文件抵消。此处没有执行生产迁移。
- [#135 规格](../plans/2026-09-09-mixed-sources-spec.md)记录范围与门禁。运行/发布/原件接缝的受控模型与语义报告替身不等于真实模型关联200/220或冲突澄清；真实语义与搜索资格单列，不能以工程测试代替。
  同提交 CI 与两轴审查通过后才可合并关闭，实际状态以关联 PR 为准。
