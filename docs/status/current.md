...
>
> last_verified: 2026-09-08（#131 图片与扫描件工程实现；合并状态现场核对 GitHub）
>
...
- 当前整改权威为 [#113](https://github.com/Eclipseic1848/Mangrove_ai/issues/113)。
  #130 已由 PR #166 合并关闭；本分支实现 #131，依赖 #127、#129 已关闭。后续按工单依赖继续，不代表整个 P1 完成。
- PNG、JPEG、WEBP 静态图片与扫描 PDF 复用统一文件任务和原件预览；保留原件字节、Owner 和冻结身份。
  图片最多 1600 万像素；PDF 最多 1000 页且逐页限制渲染像素。损坏、加密、伪装和超限拒绝。
- OCR 未知分数不伪造为满分；未核坐标只保留页级待复核内容，正式证据拒绝未复核或低置信元素。
  已核坐标仅适用声明的服务版本与响应路径，表格区域不代表单元格定位。
- [#131 规格](../plans/2026-09-08-image-scan-file-task-spec.md)记录范围和门禁；真实合成 OCR、隔离 HTTP 合同和浏览器证据分别记录。
  同提交 CI、两轴审查通过后才可合并关闭，实际状态以关联 PR 为准；没有授予生产、真实业务或普遍 OCR 精度资格。
