...
>
> last_verified: 2026-09-08（#133 已批准工具复用工程实现；合并状态现场核对 GitHub）
>
...
- 当前整改权威为 [#113](https://github.com/Eclipseic1848/Mangrove_ai/issues/113)。
  #132 已由 PR #169 合并关闭；本分支实现 #133，依赖 #125、#126 已关闭。后续按工单依赖继续，不代表整个 P1 完成。
- 当前灰度工具卡片可显式复用兼容的已批准精确版本；服务端核对实际输入/输出与当前治理，冻结引用并记录选型。普通用户受众不扩大。
- 未命中后的公开候选检索需要明确确认，只发送通用需求标签，不自动安装、晋级、发布或给临时脚本永久资格。
- CoreMind 沿自身冻结工具合同调用隔离 Host；每次核对 Owner、版本、治理、只读声明、参数 Schema 和 MCP 工具名单。模型仅收到调用合同投影。
- 无文件写入的只读调用不伪造文件检查点；本地 pending/returned 记录阻止未知重放，候选写入继续走原检查点及独立交付门。取消清理本 Run 的 Worker 和 Host。
- [#133 规格](../plans/2026-09-08-approved-tool-reuse-spec.md)记录范围与门禁。真实 SDK/Worker/Host 的固定合成验证使用本地脚本化模型响应，不代表模型自主能力、完整目录治理或正式交付资格。
  正向恢复、同提交 CI、两轴审查通过后才可合并关闭，实际状态以关联 PR 为准。
- 历史 G1 资格绑定原冻结源码；本票不改冻结数据、不授予新 G1 资格，不执行生产迁移、部署或发布。
