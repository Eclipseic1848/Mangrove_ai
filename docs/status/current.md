...
- G2 剩余门：AC-05 隔离能力获取的生产迁移与用户验收；Word/Excel 连续 3/3 已通过，
  但该局部结果不等于完整 PG-05 完成。
...
2026-08-22，G2 Office 连续生产门通过，完整 G2 仍未完成：
...
- AC-05 生产迁移、备份与源库一致性、既有治理表零改写及用户验收尚未执行，因此 Issue #38、
  完整 G2 和 Phase 4 均保持未完成。

## 7. 当前优先顺序

1. **#40 G1 本地工程门已达标**：v3 正式结果功能 96.8%、安全 100%，资格 PASS；下一步是
   在用户单独授权后更新/关闭 Issue、推送分支并建 PR。当前本地结果不得冒充远端工单已完成。
2. **G2 部分完成、G3 未开工**：G2 Office 3/3 已通过，下一门是 AC-05 生产迁移与用户验收；
   G3 默认入口切换仍需独立规格、实现与用户授权。
