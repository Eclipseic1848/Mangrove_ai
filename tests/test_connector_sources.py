...
def test_sqlite_read_freezes_owner_version_and_snapshot(tmp_path):
    ...
    spec = SourceSpec(
        ...
        options={
            ...
            "task_id": "attempt-a",
            "incremental": {"cursor_field": "water"},
        },
    )
    ...
    content = b"".join(
        store.read_raw_bytes("attempt-a", artifact["storage_path"])
        for artifact in result["artifacts"]
    )
    assert [item["id"] for item in map(json.loads, content.splitlines())] == [1, 2, 3]
...
def test_http_paged_raw_with_pinned_transport_and_read_only_scope(tmp_path):
    ...
    spec = SourceSpec(
        ...
        locator="https://fixture.invalid/data",
        options={
            "task_id": "attempt-http",
            "pagination": {"strategy": "page", "options": {"max_pages": 4}},
        },
    )
    row = {
        "owner": "a",
...
@pytest.mark.parametrize(
    "sql",
    ["DELETE FROM t", "DROP TABLE t", "CALL p()", "SELECT side_effect()"],
)
...
def test_owner_and_changed_version_reject_before_read(tmp_path):
    ...
    row["version"] = "2"
    with pytest.raises(PermissionError):
        asyncio.run(read_frozen("a", frozen, lambda: row, None, None))
...
def test_cancel_waits_for_actual_database_thread(tmp_path):
    ...
    async def run():
        task = asyncio.create_task(
            read_frozen("a", freeze("a", row), lambda: row, Slow(), None)
        )
        try:
...
def test_http_error_retains_classification(tmp_path, status):
    ...
    spec = SourceSpec(
        ...
        source_type=SourceType.HTTP_API,
        locator="https://fixture.invalid/data",
        options={"task_id": "a", "max_retries": 0},
    )
    row = {"owner": "a", "connection_id": "a", "version": "1", "spec": spec}
    connector = ReadOnlyHttpConnector(
        url=spec.locator,
        artifact_store=store,
        security_guard=HttpSecurityGuard(resolver=lambda host: ["93.184.216.34"]),
...
@pytest.mark.parametrize(
    ...
    [
        (b"[]", 200, "no_results"),
        (b"not json", 200, "local_parse"),
        (b"", 401, "authorization_expired"),
        (b"", 403, "permission_denied"),
    ],
)
def test_precise_http_empty_parse_and_permission(tmp_path, body, status, expected):
    ...
    if expected == "local_parse":
        assert not list((tmp_path / "http").rglob("raw-*"))
...
def test_database_resume_repeated_watermark_uses_frozen_checkpoint(tmp_path):
    ...
    with sqlite3.connect(path) as db:
        ...
        db.executemany("INSERT INTO t VALUES(?,7)", [(number,) for number in range(1, 7)])
    spec = SourceSpec(
        source_id="x",
        source_type=SourceType.DATABASE,
        locator="dbconn://a",
    ...
    saved = {
        ...
        "value": batch.checkpoint.to_dict(),
    }
    result = asyncio.run(
        read_frozen(
            "a",
            frozen,
            lambda: row,
            DatabaseConnector(store),
            store,
            checkpoint=saved,
        )
    )
    artifacts = [artifact.model_dump(mode="json") for artifact in batch.artifacts]
    artifacts += result["artifacts"]
    rows = [
        json.loads(line)
        for artifact in artifacts
        for line in store.read_raw_bytes(
            "resume",
...
def test_isolated_real_http_service_pagination_and_cleanup(tmp_path):
    ...
    try:
        ...
        row = {
            "owner": "a",
            "connection_id": "synthetic-http",
            "version": "1",
            "spec": spec,
        }
        store = ArtifactStore(str(tmp_path / "raw"))
        # 仅此隔离 fixture 显式回环许可；生产默认守卫仍拒绝回环。
        guard = HttpSecurityGuard(
            allow_private=True,
            loopback_host_allowlist=("127.0.0.1",),
        )
        connector = ReadOnlyHttpConnector(
            url=url,
            artifact_store=store,
        ...
        result = asyncio.run(
            read_frozen("a", freeze("a", row), lambda: row, connector, store)
        )
...
def test_raw_cleanup_rejects_wrong_hash_and_never_touches_other_namespace(tmp_path):
    ...
    path = store.resolve_path(raw.storage_path)
    path.write_bytes(b"changed")
    with pytest.raises(ValueError, match="identity_changed"):
        remove_connector_raw(
            {"artifact_namespace": "target"},
...
@pytest.mark.parametrize(
    "status,expected,calls_expected",
    [
        (302, "source_redirect_denied", 1),
        (503, "source_network", 1),
        (200, "source_scope_denied", 0),
    ],
)
def test_http_scope_redirect_and_zero_retry(
    tmp_path,
    status,
    expected,
    calls_expected,
    ...
    spec = SourceSpec(
        ...
        source_type=SourceType.HTTP_API,
        locator="https://fixture.invalid/data",
        options={"task_id": "scope"},
    )
    row = {"owner": "a", "connection_id": "a", "version": "1", "spec": spec}
    guard = HttpSecurityGuard(
        resolver=lambda host: ["127.0.0.1" if status == 200 else "93.184.216.34"]
    )
    connector = ReadOnlyHttpConnector(
        url=spec.locator,
        artifact_store=store,
...
def test_http_checkpoint_duplicate_is_not_counted_twice(tmp_path, monkeypatch):
    ...
    request = ConnectorRequest(freeze("owner-a", row), "读取")
    store = ArtifactStore(str(tmp_path / "raw"))
    calls = []
    ...
    def authorize(auth):
        with sqlite3.connect(database) as db:
            ...
            require_authorized(db, auth)

    with execution_validation(authorize), execution_context(
        ExecutionAuthorization("owner-a", 0)
    ):
...
@pytest.mark.parametrize(
    "url",
    [
        "https://synthetic-user:synthetic-password@fixture.invalid/data",
        "file:///synthetic-private-path",
        "https://fixture.invalid:bad/data",
    ],
)
def test_invalid_http_url_api_never_creates_attempt(tmp_path, url):
    ...
    def no_connector(row):
        raise AssertionError("非法URL不得构造读取器")

    app = FastAPI()
    app.include_router(router_for(repository, lambda *args: row, no_connector, None))
