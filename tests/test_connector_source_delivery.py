"""真实 Manager/Publisher/临时原件；仅 Runtime 生成与验证为既有替身。"""
...
def test_connector_enters_real_manager_publisher_and_source_export(
    tmp_path,
    monkeypatch,
):
    ...
    from tests.test_source_deletion_delivery import (
        DeletionFixtureRuntime as CoverageAwareWebPiRuntime,
    )
    ...
    with sqlite3.connect(path) as db:
        ...
        db.execute("INSERT INTO t VALUES(1,'甲')")
    spec = SourceSpec(
        source_id="x",
        source_type=SourceType.DATABASE,
        locator="dbconn://synthetic",
        options={
            "table": "t",
    ...
    with client:
        with execution_context(ExecutionAuthorization("user-a", 0)):
            source = asyncio.run(
                acquire_connector(
                    ...
                    owner="user-a",
                    key="source",
                    request=ConnectorRequest(freeze("user-a", row), "读取数据库"),
        ...
        archive = client.get(
            f"/api/semantic-workspace/tasks/{task}/source-bundle",
            params={"revision": 1},
        )
        assert archive.status_code == 200, archive.text
        with zipfile.ZipFile(io.BytesIO(archive.content)) as bundle:
            manifest = json.loads(bundle.read("manifest.json"))
            provenance = manifest["sources"][0]["provenance"]
            assert provenance["source_kind"] == "connector"
            assert provenance["connection_version"] == freeze("user-a", row).version
            original = next(
                file
                for file in manifest["sources"][0]["files"]
        ...
        async def refreshed(repository, **kwargs):
            return await original_refresh(
                repository,
                **kwargs,
                resolver=lambda owner, source, key: row,
                factory=lambda binding: DatabaseConnector(store),
                artifact_store=store,
            )
        ...
        updated = client.post(
            f"/api/semantic-workspace/tasks/{task}/source-refresh",
            headers={"Idempotency-Key": "refresh"},
            json={
                "expected_active_revision": 1,
                "target_source_snapshot_id": source["snapshot_id"],
            },
        )
        ...
        assert client.post(
            f"/api/semantic-workspace/tasks/{task}/restore"
        ).status_code == 200
        restored = client.get(
            f"/api/semantic-workspace/tasks/{task}/source-bundle",
            params={"revision": 1},
        ...
        assert not list((execution_root / "frozen-web-sources").rglob("*.jsonl"))
