...
def test_forward_backfills_without_rewriting_roots_and_restore_is_exact(tmp_path):
    ...
    assert migrations.inspect_database(target).pending_revisions == ('webui_0012', 'webui_0013', 'webui_0014')
    ...
    assert receipt.applied_revisions == ('webui_0012', 'webui_0013', 'webui_0014')
    with closing(sqlite3.connect(database)) as conn:
