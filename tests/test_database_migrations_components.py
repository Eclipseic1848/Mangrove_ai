...
def test_webui_0010_preserves_source_history_and_supports_recovery(tmp_path: Path) -> None:
    ...
    assert inspect_database(target).pending_revisions == ("webui_0010", "webui_0011")
    ...
    assert receipt.applied_revisions == ("webui_0010", "webui_0011")
    with closing(sqlite3.connect(database)) as connection:
