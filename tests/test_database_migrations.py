...
def _remove_platform_session_schema(connection: sqlite3.Connection) -> None:
    # 从当前空库构造历史未盖戳库，移除历史尚不存在的会话和执行授权结构。
    for table in ("feedback_content_access", "account_execution_holds", "account_execution_bindings", "platform_spent_refresh", "platform_login_sessions", "platform_rate_events", "platform_rate_blocks", "platform_security_audit"):
        assert connection.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0] == 0
        connection.execute(f"DROP TABLE {table}")
    for table in ("users", "delivery_publish_intents"):
        connection.execute(f"ALTER TABLE {table} DROP COLUMN execution_generation")
    # 合成历史库不得残留后来才有的列，更不能删除有引用证据的真实回合。
    assert connection.execute("SELECT COUNT(*) FROM conversation_raw_turns WHERE result_context_json IS NOT NULL OR result_context_claimed<>0").fetchone()[0] == 0
    connection.execute("ALTER TABLE conversation_raw_turns DROP COLUMN result_context_claimed")
    connection.execute("ALTER TABLE conversation_raw_turns DROP COLUMN result_context_json")
...
def test_inspect_uninitialized_database_is_read_only(tmp_path: Path) -> None:
    ...
    assert status.target_revision == "webui_0020"
    assert status.pending_revisions == (
        "webui_0001", "webui_0002", "webui_0003", "webui_0004", "webui_0005",
        "webui_0006", "webui_0007", "webui_0008", "webui_0009", "webui_0010", "webui_0011", "webui_0012", "webui_0013", "webui_0014", "webui_0015", "webui_0016", "webui_0017", "webui_0018", "webui_0019", "webui_0020",
    )
...
def test_plan_lists_full_ordered_revision_chain_without_writing(
    ...
    assert [item.revision for item in plan.revisions] == [
        ...
        "webui_0005",
        "webui_0006",
        "webui_0007", "webui_0008", "webui_0009", "webui_0010", "webui_0011", "webui_0012", "webui_0013", "webui_0014", "webui_0015", "webui_0016", "webui_0017", "webui_0018", "webui_0019", "webui_0020",
    ]
...
def test_inspect_current_database_is_read_only(tmp_path: Path) -> None:
    ...
    assert status.current_revision == "webui_0020"
    assert status.pending_revisions == ()
    status.require_current()
...
def test_apply_empty_database_creates_backup_and_current_revision(
    ...
    assert receipt.target_revision == "webui_0020"
...
def test_scheduler_store_requires_and_uses_explicit_migration(
    ...
    with scheduler_owner(tmp_path / "scheduler-users.db") as owner:
        task_id = store.add(owner_user_id=owner["user_id"],
            user_input="每天汇总",
            provider=None,
            model=None,
