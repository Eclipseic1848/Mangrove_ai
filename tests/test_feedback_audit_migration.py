...
def test_feedback_audit_upgrade_preserves_history_and_restore(tmp_path):
    ...
    expected = ('webui_0013', 'webui_0014', 'webui_0015', 'webui_0016', 'webui_0017', 'webui_0018', 'webui_0019', 'webui_0020', 'webui_0021', 'webui_0022')
    assert migrations.inspect_database(target).pending_revisions == expected
    receipt = migrations.apply_migrations(target, tmp_path / 'backup.db', expected_source_sha256=hashlib.sha256(before).hexdigest())
    assert receipt.applied_revisions == expected
    with closing(sqlite3.connect(database)) as conn:
        assert [row[:10] for row in conn.execute('SELECT * FROM message_feedback')] == history
