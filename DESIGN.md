...
来源与结果共用文件侧栏。同一用户、任务、修订和交付内，切换文件、输出或关闭侧栏保留各自筛选、
分页、缩放与实际横纵滚动；跨身份重置。来源表按冻结工作表和物理行定位，文档按真实元素或 PDF
页定位；未找到、解析版本变化和读取失败分别说明，不能用相同值推测命中。网页只展示已保存摘要。

正式结果中的“围绕此结果追问”把服务端登记的结果身份附到现有输入，保留用户文字，发送才执行。
仅最新修订可新建引用，历史仍可查看来源和下载。“本次追问引用的结果/来源”说明本次输入关联，
不声称逐句核验；自由回答不推断引用。多来源沿用原生选择控件，长表与长文均保留完整窗口浏览入口。
