...
全局导航使用独立深青底，与任务列表和主内容区区分；浅色模式 `#123C3A`，深色模式 `#0D2928`。
导航变量由 `index.css` 的 `--nav-*` 统一维护。当前页同时使用背景、左侧指示线和加粗文字。
工作空间与管理分组；管理顺序为运营与审计、设置、反馈管理、用户管理，除设置外均按管理员权限显示。
桌面支持收起；窄屏使用抽屉。主题图标按钮与账号同行，不再单独占一行；悬停或键盘聚焦提示目标主题。收起时仍可切换，退出失败不得清空身份或输入。

## Typography

运营与审计沿用共享按钮、原生筛选、分页与 Radix 弹窗。五个子页面共享时间和筛选，
图表下钻保留范围；明细只读，详情与导出均留痕。管理员与超管的可见范围由服务端判定。
空数据明确显示采集起点，未知结果不冒充成功；窄屏筛选换行、宽表内部滚动。
按确认原型采用紧凑指标带、趋势与关注事项双列、分类型明细表；事件详情与用户时间线从右侧展开。
列表使用分页与限高滚动，不随记录数拉长整页；外层应用固定在视口，导航不随页面滚出窗口。
...
业务澄清和真实“当前理解”留在对话主列，一次突出一个问题，沿用同一个多行输入。用户明确切换
“回答这项问题”或“继续对话”；普通草稿和结果引用不会被静默转成答案。选项来自服务端，来源发现
只展示已核实的有界观察。执行摘要标为执行状态，不能冒充业务理解。问题可收起重开，长选项可换行。

回答使用服务端问题轮次和修订身份；失败或结果未知保留原稿，不自动重发。旧响应不清新问题或新稿。
已答历史从持久记录恢复。外发许可继续使用专门确认，保留原用途说明；普通暂停只显示控制等待。
已冻结 Pi/CoreMind 补充后仍需明确确认“按补充要求重新开始”，创建同任务新版本、新执行，不称恢复原 Run。
