...
>
> 当前分支：`codex/ac07-34-capability-host-proxy`
>
> 当前公开远端：`origin` → `https://github.com/Eclipseic1848/Mangrove_ai.git`
>
> 当前公开基线：本地 `main`、`HEAD` 与 `origin/main` 均为 `e72ca4c9b1771b3a3b303131b5705783e725db2a`
>
> 版本状态：`v0.0.8` 的当前能力已纳入首次公开快照；没有创建或移动版本标签

## 0. 一句话结论

AC-07 #35 已经 PR #6 合并到 `main`，旧仓库 Issue #35 已关闭。AC-07 #34 已完成两项
真实能力任务、正常验证、取消后重新发起、证据/资源清理和 8088 用户验收。真实任务同时暴露并修复了
Pi 内部 Capability Host 请求被外发代理接管的问题；当前仅 `src/agentic_runtime/pi_runtime.py`、
`tests/test_agentic_runtime.py` 和两份状态文档是未提交变更。下一工程门是对该窄范围修复做双轴审查；
工单操作、Commit/Push、签名密钥、能力晋级或权限扩大都必须重新取得用户明确授权。
...
当前 `main` 已包含 #35 的合并结果；未提交工作树是 #34 灰度暴露的 Capability Host 代理绕过修复、
回归测试和本次状态文档。接手时仍必须现场核对 `git status`、分支、远端与工单状态。未授权修改旧 Issue、
提交、推送、晋级或发布能力，也不得使用 `git add .` 处理后续混合工作树。
...
| AC-07 #34 | 精确 digest 的可恢复 ValidationRun、Owner/TaskRevision 绑定、真实 Pi 隔离重放、独立 Verifier、幂等/Lease、取消后重新发起、失败关闭清理、两项真实能力灰度和 8088 用户验收已完成 | Capability Host 代理窄绕过修复待双轴审查和主线集成；旧 Issue #34 仍开放 |
| AC-07 #35 | 固定 Trivy 0.70.0、Syft 1.50.0，最终主体扫描、双格式 SBOM、不可变证据、精确隔离门和脱敏 UI 已实现；最终双轴复审、带备份生产迁移与 8088 用户验收通过；PR #6 已合并且旧 Issue #35 已关闭 | 无 |
...
- PR #6 已以 merge commit `e72ca4c9b1771b3a3b303131b5705783e725db2a` 合并；本地
  `main`、`HEAD` 与 `origin/main` 一致。首次公开快照共 1163 个文件，0 个 Gitlink。
...
- #34 真实 Python Tool 与 Everything MCP 任务均记录目标 `tool.completed`；两条
  ValidationRun 五步全部通过，另一条取消运行仅保留 `cleanup=passed`。用户已确认
  #34 的 8088 验收通过；最终 Lease、容器、网络和运行目录零残留。

## 4. 当前卡在哪里

### 4.1 没有代码级硬阻塞，主要是人工门禁

当前没有需要靠猜测继续编码的故障。#35 已合并并关闭旧工单；#34 的真实灰度与用户验收已完成，
但灰度暴露的 Capability Host 代理绕过修复仍在未提交工作树中，必须先审查再决定集成。旧工单处理和
后续工单仍必须保持人工控制。局部闭环不能被表述为能力已经晋级、签名、平台发布，或整个 AC-07/Phase 4 已完成。

### 4.2 #34 真实灰度已完成，修复待审查和集成

#34 已使用管理员自有 TaskRevision 完成 `gray-python-table@1.0.0` 与
`gray-everything-mcp@2026.7.4` 真实原生调用，两条成功 ValidationRun 五步全部通过；
另一条 MCP 运行验证了取消、取消后清理和使用新幂等键重新发起。用户已确认 8088 验收通过。

灰度中发现 Pi 的内置 `fetch` 因空 `NO_PROXY` 将任务内 Capability Host 请求送入业务外发代理；
当前工作树以“仅绕过本任务 Host DNS”的最小修复解决，相关回归 `50 passed`。该修复尚未审查、
提交或进入公开主线，因此不得仅根据本地验收关闭工单。
...
5. 面向 `main` 的 PR #6 已合并，旧仓库 Issue #35 已关闭。

本票没有晋级能力、生成 Cosign 密钥、发布平台快照或扩大普通用户受众。

### 4.4 工单跟踪已经分成两个仓库

- 代码的新权威仓库是公开的 `Eclipseic1848/Mangrove_ai`。
- AC-07 #32～#44 仍存在旧的 `Eclipseic1848/Mangrove_platform`，由本机
  `legacy-platform` 远端对应；#33 与 #35 已关闭，#34 仍开放。
...
### 下一人工控制点：审查 #34 Capability Host 代理绕过修复

1. 以当前 `main`/`e72ca4c9` 为固定点，对未提交代码与测试执行 Standards/Spec 双轴审查。
2. 审查通过后，再由用户单独决定 Commit、Push 和旧 Issue #34 处理；不得把文档同步视为授权。
3. #34 修复进入主线后，再决定是否进入 #36 Cosign 本地 OCI 签名路径 PoC。
...
| 2 | #34 | 精确 digest 的可恢复 ValidationRun | 真实灰度和用户验收完成；代理绕过修复待审查、集成和旧工单处理 |
| 3 | #35 | Trivy、Syft 供应链证据闭环 | 工程、审查、生产迁移和用户验收完成；PR #6 已合并，旧 Issue #35 已关闭 |
...
- #34 与 #35 的本地事实闭环已完成；先审查并集成 #34 灰度暴露的代理绕过修复；
