...
>
> 当前阶段：**#16 已真实完成（阶段 0-6 + 收口中）**；下一任务 #17
> （AC-06 兼容切换与 AC-07 综合验收门）停在授权门
...
下一步是 #17（AC-06 兼容切换与 AC-07 综合验收门），停在用户授权门。
...
**没有代码级硬阻塞**。#15/#16 已真实完成（#16 阶段 0-6 + AC 对照 + 执行报告；
发布链收口中）。当前停在「#17（AC-06 兼容切换与 AC-07 综合验收门）授权门」：

1. #16 全部交付：AC1-AC7 对照 ✅（执行报告
   `docs/plans/2026-08-19-agentic-capability-ac07-11-execution-report.md`）、
   真实缺陷修复 5 个、回归 305 passed；
2. #17 依赖 #15/#16 已满足（两条真实纵切面齐备）；计划未展示，停在授权门；
3. #16 发布链（分支→提交→推送→PR）进行中；合并待确认。
...
### #16 已完成（2026-08-19，阶段 0-6 + AC 对照 + 执行报告；发布链收口中）

Everything MCP 纵切面真实走通：三类验证（含真实 MCP echo Smoke 增量）、单 Sidecar 双能力
装载、篡改→自动隔离→restore、跨 Owner 拒绝、并行版本 2026.8.19（发布→deprecated→revoked
牺牲）、协议纵深（超时/取消/进程异常 fail-closed）、零残留（Lease 0/容器 0/无 MCP 子进程
残留）。详见 §3.4 与执行报告。

### #17 AC-06 兼容切换与 AC-07 综合验收门（下一步，待授权）
