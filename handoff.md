...
#135 已由 PR #172 合并关闭。#136 已完成三类本人历史资料直接复用、原件预览与真实引用展示的工程实现；规格见 docs/plans/2026-09-09-reusable-sources-spec.md。原上传、网页快照与正式输出保持原身份及摘要，支持混合新资料，不重复上传或抓取。正文读取前登记使用并锁定，默认修订固定到同一响应；失效原件拒绝缓存绕过。正式消费关系在任务记录清理后仍保留。仅临时数据库验证 webui_0017，未迁移生产；物理清理属于 #137，须保留最小来源身份映射。两轴本地复核无未修阻断；实际 CI、合并状态以关联 PR 为准，替身生成与传输不证明真实模型或搜索资格。

门禁通过并关闭 #136 后，按依赖继续 #137，不重做前票。
