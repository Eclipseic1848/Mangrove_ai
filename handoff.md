...
G4 已完成本地工程门实现与 74 项定向回归，但平台 DeepSeek/Qwen 的真实 Pi 合成请求、生产
Vault 两阶段轮换和最终三证据汇总均未执行；G5（8B/Linux 服务器）仍未就绪。
...
- **G4 工程门已实现、生产门待执行；G5 挂起**：G4 已获准使用 DeepSeek/Qwen 纯合成数据，
  但真实 Pi 请求和生产 Vault 轮换未执行；8B/Linux 目标服务器未就绪（不得表述为已完成）。
...
- G4：执行范围限定为平台共享 DeepSeek/百炼连接，外发仅限冻结的纯合成数据；
  ...
  定向回归已通过，真实生产顺序固定为：先提交并绑定干净 SHA；运行传输矩阵；停服后准备
  双代际轮换；重启加载双代际后再次停服；核验旧 PID/端口、配置的 `data/backups` 目录和其中
  全部含 Provider Secret 的数据库备份后最终轮换；重启；运行 DeepSeek/
  百炼真实 Pi 合成链；汇总三份同身份报告。提交、不可逆生产轮换、默认切换仍是独立授权门；
- G5：用户目标服务器就绪并授权部署验证。
