...
> 当前分支：`main`
>
> 当前提交：`f1d1906e`（文档同步 PR #21）；#11 变更随本 PR 提交，合并 SHA 以现场为准
>
> 公开远端：`origin` → `https://github.com/Eclipseic1848/Mangrove_ai.git`
>
> 当前阶段：AC-07 能力信任与发布治理；#9/#10/#11 已关闭，下一工单为 #12

## 0. 一句话结论

Mangrove 正在把“能运行的个人能力”推进为“证据完整、可审计、可失败关闭、以后可以由管理员
发布的平台能力”。AC-07 的验证运行、供应链扫描、本地标准 OCI 签名、个人能力自动晋级机制
与管理员审核/审计查看已经完成；下一步是新仓库 #12“独立平台快照、签名与 admin_gray 发布”。

不要把 #9/#10/#11 完成、测试通过、Capability 可运行或管理员灰度可用表述成能力已经自动晋级、
...
5. 当前工单 #12、AC-07 规格、ADR-0029，以及旧
   `Eclipseic1848/Mangrove_platform#34/#35`、新 `Eclipseic1848/Mangrove_ai#9/#10/#11` 的执行报告。
...
git remote -v
gh issue view 12 --repo Eclipseic1848/Mangrove_ai --comments
```

预期现场状态是：

- 本地 `main`、`HEAD` 与 `origin/main` 均为 #11 PR 的合并提交（SHA 以现场为准）；
- 工作树应干净；
- 新仓库 #9、#10、#11 为 `CLOSED / COMPLETED`；
- 新仓库 #12～#17 为开放工单；
...
| 新仓库 #11 | 管理员审核与业务内容审计查看：跨 Owner 审核聚合、三组分组、渐进披露、审计查看命令（原因必填、任务与验证证据绑定、失败留痕、2MiB 按块截断）、零 DDL、双轴审查修复复核、8088 用户验收完成 | 无；Issue 待关闭（发布动作授权后）；平台候选/管理员灰度分组留待 #12 |
...
没有已知代码级硬阻塞，也没有在运行的实现任务。当前停在“进入 #12 前的人控阶段门”：

1. #12 Issue 已存在，但新会话仍要核对其规格与当前代码是否漂移。
2. 用户尚未授权 #12 开工；不要把“下一任务是 #12”理解为已授权实现。
3. #12 涉及独立平台快照、签名与 `admin_gray` 发布（#9 的 Cosign/ORAS/Zot 工具链是其前置），
   发布语义与受众必须由用户确认。
...
工单：`Eclipseic1848/Mangrove_ai#12`，独立平台快照、签名与 `admin_gray` 发布。

### 5.1 目标

从 verified 个人版本生成独立脱敏平台快照、新 digest 与独立生命周期，Cosign 签名后按
`admin_gray` 受众发布；个人 scope 不被修改，个人后续修改/弃用/删除不影响平台快照。

### 5.2 参考流程

沿用 #10/#11 的纵向流程：需求/规格复核（对照 Issue #12、AC-07 规格 §6/§7、ADR-0029
决策 6 与当前 Repository）→ 领域/接口设计 → 任务拆分 → TDD 实现 → 冻结夹具验证 →
...
- #11 的管理员审核与审计查看已随本 PR 完成，不在 #12 重复实现。
...
| 6 | 新仓库 #11 | 管理员审核与业务内容审计查看 | 已完成；Issue 待关闭（发布动作授权后）；平台候选/管理员灰度分组留待 #12 |
| 7 | 新仓库 #12 | 独立平台快照、签名与 `admin_gray` 发布 | **下一工单**；依赖 #9、#10、#11，均已具备 |
...
读取上述资料和 Issue #12 后，先给用户一份只读阶段判断，不要立即实现。至少说明：

1. 当前阶段：#12 开工前的需求/规格复核。
2. 已验证事实：#9/#10/#11 已关闭、#12 开放、前置全部具备、当前 Git 实况。
3. 基于代码的推断：现有 #9 的 Cosign/ORAS/Zot 工具链与治理 Repository 中哪些 Seam 可复用。
4. 尚未验证的建议：#12 的最小纵向切片、测试策略和是否需要数据库变化。
5. 必须由用户确认：平台快照语义、脱敏边界、签名身份、发布受众、迁移、真实任务、外部模型、
   Commit/Push/PR/Issue 写入。
6. 根据开发计划，#12 完成后的下一任务是 #13；但不得自动进入。

如果用户只说“继续”，优先完成当前已确认阶段，不要把整个 #12 拆成没有价值的微步骤，也不要
越过用户控制点。
