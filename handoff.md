...
> 当前分支：`main`
>
> 当前提交：`3a0eaeb7`（#12 PR 合并）；#13 变更尚未提交，工作树含未提交改动与未跟踪文件
>
> 公开远端：`origin` → `https://github.com/Eclipseic1848/Mangrove_ai.git`
>
> 当前阶段：AC-07 能力信任与发布治理；#9～#13 工程与验收完成，#13 待发布动作授权，下一工单为 #14

## 0. 一句话结论

Mangrove 正在把“能运行的个人能力”推进为“证据完整、可审计、可失败关闭、以后可以由管理员
发布的平台能力”。AC-07 的验证运行、供应链扫描、本地标准 OCI 签名、个人能力自动晋级机制、
管理员审核/审计查看、平台快照发布机制与运行时装载治理门已经完成；下一步是新仓库 #14
“弃用、回滚、隔离、撤销与限期风险接受”。

不要把 #9～#13 完成、测试通过、Capability 可运行或管理员灰度可用表述成能力已经自动晋级、
...
5. 当前工单 #13、AC-07 规格、ADR-0029，以及旧
   `Eclipseic1848/Mangrove_platform#34/#35`、新 `Eclipseic1848/Mangrove_ai#9～#12` 的执行报告。
...
git remote -v
gh issue view 13 --repo Eclipseic1848/Mangrove_ai --comments
```

预期现场状态是：

- 本地 `main`、`HEAD` 与 `origin/main` 均为 #12 PR 的合并提交（SHA 以现场为准）；
- 工作树应干净；
- 新仓库 #9、#10、#11、#12 为 `CLOSED / COMPLETED`；
- 新仓库 #13～#17 为开放工单；
...
没有已知代码级硬阻塞。#13 工程实现、双轴审查与 8088 验收已完成，当前停在
“#13 发布动作授权”的人控阶段门：

1. #13 全部变更在工作树（未提交）；`git status --short --untracked-files=all` 现场核对
   文件清单后再提交。
2. 发布动作（分支、提交、推送、PR、Issue 关闭）必须逐项获得用户授权。
3. #13 完成不代表任何能力已经晋级、平台已发布或普通用户受众扩大；真实治理纵切面
   留待 #15/#16。
...
工单：`Eclipseic1848/Mangrove_ai#14`，弃用、回滚、隔离、撤销与限期风险接受。

### 5.1 前置状态

#13 工程实现、双轴审查与 8088 验收已完成，发布动作（提交/推送/PR/Issue 关闭）待用户
逐项授权。授权完成后按历史 PR 流程走 `codex/ac07-08-*` 分支。

### 5.2 #14 目标（开工前仍需现场复核 Issue 与规格）

- 弃用/回滚/隔离/撤销命令与自动 quarantined 投影（#13 监督已预留 REVOKED/QUARANTINED
  检测语义，但投影变化的产生端属于 #14）。
- 撤销/隔离的治理事件形态与 validator（#13 测试夹具以 monkeypatch 投影替代，待 #14
  落地真实事件形态后切换）。
- 限期风险接受。

### 5.3 参考流程

沿用 #10～#13 的纵向流程：需求/规格复核 → 领域/接口设计 → 任务拆分 → TDD 实现 →
...
| 7 | 新仓库 #12 | 独立平台快照、签名与 `admin_gray` 发布 | 已完成；Issue 待关闭（发布动作授权后）；真实探针与真实发布纵切面留待 #15/#16 |
| 8 | 新仓库 #13 | CapabilityMountResolver 运行时治理门 | 工程实现、双轴审查与 8088 验收完成；发布动作待授权；依赖旧 #33、新 #12 |
| 9 | 新仓库 #14 | 弃用、回滚、隔离、撤销与限期风险接受 | **下一工单**；依赖 `Mangrove_platform#35`、新 #13 |
...
读取上述资料和 Issue #14 后，先给用户一份只读阶段判断，不要立即实现。至少说明：

1. 当前阶段：#13 发布动作是否已授权完成；未完成则先收口 #13，再进入 #14 需求/规格复核。
2. 已验证事实：#9～#13 状态、#14 开放、前置全部具备、当前 Git 实况。
3. 基于代码的推断：#13 运行时门的 REVOKED/QUARANTINED 检测语义与 #14 事件形态的衔接点。
4. 尚未验证的建议：#14 的最小纵向切片、测试策略和是否需要数据库变化。
5. 必须由用户确认：弃用/回滚/隔离/撤销语义、自动 quarantined 投影触发条件、风险接受期限、
   真实任务、Commit/Push/PR/Issue 写入。
6. 根据开发计划，#14 完成后的下一任务是 #15；但不得自动进入。

如果用户只说“继续”，优先完成当前已确认阶段，不要把整个 #14 拆成没有价值的微步骤，也不要
越过用户控制点。
