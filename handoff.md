...
> 当前分支：`main`
>
> 当前提交：`f1d1906e`（文档同步 PR #21）；#12 变更随本 PR 提交，合并 SHA 以现场为准
>
> 公开远端：`origin` → `https://github.com/Eclipseic1848/Mangrove_ai.git`
>
> 当前阶段：AC-07 能力信任与发布治理；#9/#10/#11/#12 已关闭，下一工单为 #13

## 0. 一句话结论

Mangrove 正在把“能运行的个人能力”推进为“证据完整、可审计、可失败关闭、以后可以由管理员
发布的平台能力”。AC-07 的验证运行、供应链扫描、本地标准 OCI 签名、个人能力自动晋级机制、
管理员审核/审计查看与平台快照发布机制已经完成；下一步是新仓库 #13“CapabilityMountResolver
运行时治理门”。

不要把 #9～#12 完成、测试通过、Capability 可运行或管理员灰度可用表述成能力已经自动晋级、
...
没有已知代码级硬阻塞，也没有在运行的实现任务。当前停在“进入 #13 前的人控阶段门”：

1. #13 Issue 已存在，但新会话仍要核对其规格与当前代码是否漂移。
2. 用户尚未授权 #13 开工；不要把“下一任务是 #13”理解为已授权实现。
3. #13 涉及 CapabilityMountResolver 运行时强制门（个人 verified/active/eligible + 平台
   受众与签名），与 #12 已建立的受众投影直接衔接，权限与运行门语义必须由用户确认。
...
工单：`Eclipseic1848/Mangrove_ai#13`，CapabilityMountResolver 运行时治理门。

### 5.1 目标

CapabilityMountResolver 作为运行时唯一装载 Seam：个人 Pack 必须 verified/active/eligible
且属于当前 Owner；平台 Pack 还必须满足受众（admin_gray/users）与 Cosign 签名。创建、重试、
恢复与每次原生能力调用都不能绕过运行门；运行中被隔离或撤销时当前原子调用完成后停止后续
调用；deprecated 只允许历史冻结任务继续装载。

### 5.2 参考流程

沿用 #10/#11/#12 的纵向流程：需求/规格复核（对照 Issue #13、AC-07 规格 §8、ADR-0029
与当前 Repository）→ 领域/接口设计 → 任务拆分 → TDD 实现 → 冻结夹具验证 →
Standards/Spec 双轴审查 → 用户验收 → 发布动作逐项授权。
...
- #12 的平台快照发布机制已随本 PR 完成，不在 #13 重复实现。
...
| 8 | 新仓库 #13 | CapabilityMountResolver 运行时治理门 | **下一工单**；依赖旧 #33、新 #12，均已具备 |
...
读取上述资料和 Issue #13 后，先给用户一份只读阶段判断，不要立即实现。至少说明：

1. 当前阶段：#13 开工前的需求/规格复核。
2. 已验证事实：#9～#12 已关闭、#13 开放、前置全部具备、当前 Git 实况。
3. 基于代码的推断：现有 CapabilityMountResolver、受众投影与 #9 签名证据中哪些 Seam 可复用。
4. 尚未验证的建议：#13 的最小纵向切片、测试策略和是否需要数据库变化。
5. 必须由用户确认：运行门语义、隔离/撤销时的原子调用边界、AC-06 兼容路径、真实任务、
   Commit/Push/PR/Issue 写入。
6. 根据开发计划，#13 完成后的下一任务是 #14；但不得自动进入。

如果用户只说“继续”，优先完成当前已确认阶段，不要把整个 #13 拆成没有价值的微步骤，也不要
越过用户控制点。
