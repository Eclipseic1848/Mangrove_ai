...
> 文档用途：写给完全没有历史对话的新会话
>
> 最后现场核验：2026-08-22（G2 Office 3/3 正式运行后）
>
> 当前分支：`codex/g1-independent-evaluation`；正式评测绑定的代码基线 = `83fe3f70`；
> 当前 G2 Office 代码提交：`31729495`；当前 HEAD 仍以现场 `git rev-parse HEAD` 为准
>
...
Word/Excel 各连续 3/3，AC-05 生产迁移与用户验收仍未执行；G3 GateSnapshot + 默认入口切换
尚未开始，切换动作仍需独立实现与用户授权。G4 已有平台配置的 DeepSeek/Qwen 连接，但真实
...
| 新 #38 | G2：PG-05 收口 | OPEN（Office 3/3 PASS；AC-05 生产迁移与用户验收未执行） |
...
2. **Phase 4 剩余门（当前）**：#40 G1 本地工程门资格 PASS；G2 Office 3/3 PASS、AC-05
   生产迁移与用户验收待执行；G3 GateSnapshot + 默认入口切换仍未实现且切换需用户单独授权；
   G4/G5 挂起。
