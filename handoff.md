...
没有代码级硬阻塞。#35 已提交并推送到 `origin/codex/ac07-35-review-fixes`，Draft PR #6 已创建
且发布门复核通过，但尚未转为 Ready、合并或关闭旧仓库工单；下一条工程主线是补齐 #34 两项
真实能力验证闭环。任何后续 PR 状态变更、合并、工单操作、签名密钥、能力晋级或权限扩大都
必须重新取得用户明确授权。
...
接手时仍必须现场核对 `git status`、分支和远端；未授权将 Draft PR #6 转为 Ready、合并、
修改旧 Issue、晋级或发布能力，也不得使用 `git add .` 处理后续混合工作树。
...
| AC-07 #35 | 固定 Trivy 0.70.0、Syft 1.50.0，最终主体扫描、双格式 SBOM、不可变证据、精确隔离门和脱敏 UI 已实现；最终双轴复审、带备份生产迁移与 8088 用户验收通过；Draft PR #6 已创建且发布门复核通过 | 转 Ready、合并和旧仓库 Issue #35 处理需分别授权 |
...
- Draft PR #6 发布门复核：七个 Capability 测试文件 `118 passed`，设置页 Playwright
...
Draft PR #6；转 Ready、合并、旧工单处理和后续工单仍必须保持人工控制。局部闭环不能被表述为
能力已经晋级、签名、平台发布，或整个 AC-07/Phase 4 已完成。
...
5. 面向 `main` 的 Draft PR #6 已创建，发布门现场复核通过。

尚未完成：Draft PR 转 Ready、合并和旧仓库 Issue #35 处理。本票没有晋级能力、生成 Cosign
密钥、发布平台快照或扩大普通用户受众。
...
### 下一人工控制点：完成 #35 Draft PR 发布流程

1. 面向 `main` 的 Draft PR #6 已创建且发布门复核通过；不要自动转 Ready 或合并。
2. 状态文档的提交和推送仍是两个独立 Git 动作，必须分别取得用户授权并使用明确文件白名单。
3. Draft 转 Ready、合并与旧仓库 `Eclipseic1848/Mangrove_platform` 的 Issue #35 处理是三个
   独立外部动作，必须分别授权；不得把旧编号静默解释为公开仓库工单。
...
| 3 | #35 | Trivy、Syft 供应链证据闭环 | 工程、审查、生产迁移和用户验收完成；Draft PR #6 已创建且发布门复核通过，待转 Ready、合并与旧工单处理授权 |
