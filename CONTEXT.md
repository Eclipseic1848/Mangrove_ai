...
任务所有者手工填写并验证，或未来由独立登录流程建立的认证来源配置；它只保存该 Owner 隔离的
SecretRef 与当前连接身份，由该 Owner 的后续任务复用直到登录状态再次失效。管理员可配置全局
兜底，但本人配置优先；不同 Owner 不共享 Cookie、SecretRef 或浏览器上下文。替换或清除时删除
旧 Cookie 密文，只保留无秘密审计事实。
_Avoid_: 跨用户共享、共享浏览器 Profile、把全局兜底当本人配置、一次验证永久有效

**认证来源会话（AuthenticatedSourceSession）**：
任务所有者为一个任务建立的隔离、限时且可版本化的来源登录状态；它只授予约定范围内的
只读访问，不向任务、模型或产物暴露密码、Cookie 或 Token。需要认证但尚无可验证 Cookie 时，
不得进入需要该来源的任务主链。手工更新或未来登录流程形成的新状态写回该 Owner 的
AuthenticatedSourceConnection，并供其后续任务使用。
...
确定失效必须有平台登录页、未登录响应、明确鉴权拒绝或身份检查失败等证据。当前由任务所有者
手工更换本人 Cookie 后显式继续；未来若提供扫码，也必须保存到该 Owner 的隔离连接。网络超时、
平台故障、VPN、WAF、验证码或反爬拦截只表示状态无法确认，不得删除现有 Cookie 或自动重放。
同一 Owner 的其他暂停任务可以复用更新后的连接身份，但必须逐个明确继续；更新 Cookie 本身不
自动启动任务，也不创建新任务。服务重启后仍从原持久身份恢复。
