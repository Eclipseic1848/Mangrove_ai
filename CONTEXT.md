...
**外部只读边界（ExternalReadOnlyBoundary）**：
Mangrove 按任务授权读取外部来源和调用所选模型，不修改外部数据库、同步业务系统或执行社交互动。
ADR-0044 仅允许用户明确授权后通过邮件或 Slack 发送本任务结果；资料中的发送指令不构成授权，
不开放任意外部写工具。平台内部生成结果与任务内发布不属于外部系统写入。
_Avoid_: 任意外部系统写入、后期通用 Sink、扫码即获得账号全部操作权限
...
任务所有者对来源获取授权的网络范围；精确网址默认不授权跟链，而明确的搜索任务可以在获准
主题、时间和有界来源范围内跨站检索与读取，扩大范围或使用认证来源仍须另行授权。
_Avoid_: 输入网址即全站授权、无限跟链、要求搜索用户先提供全部网址
...
**可复用任务数据（ReusableTaskData）**：
由 Owner 控制、能被后续任务引用的原始制品或处理结果，保留来源与加工关系，引用不扩大访问
权限；原件与派生表示分开，默认保留直到 Owner 主动删除。
_Avoid_: 第二份任务正文、跨用户共享缓存、转换后删除原件、只保留摘要或网址

**关联数据删除（ReferencedDataDeletion）**：
删除任务时一并处理未被其他任务引用的数据；对已被引用的数据，必须由 Owner 确认是否删除，
确认后先暂停依赖执行，保留其他任务与独立结果并明确标记来源不可用。
_Avoid_: 无提示级联删除、只删记录却声称数据已删除、删除后仍声称可完整重验
...
用户自然语言明确指定的格式优先于系统推荐；本会话前轮用户的明确要求可继承，本轮更正优先。
系统推荐与用户手动选择分开记录，只有手动选择与文字要求确实冲突才需要确认；输入文件后缀不决定输出格式。
...
输出制品和 Manifest。
ADR-0042 增加明确的“用户接受初稿”放行依据：在新修订保留接受记录，验证状态仍为
`inconclusive`；文件完整性 QA、来源/Owner 和取消门不豁免，不冒充系统验证通过。
ADR-0047 将工作台 Pi 初稿后的默认行为改为等待用户决定；只有明确继续核对才恢复原运行，
刷新与重启不能替用户批准。接受初稿仍沿用上述发布边界。
...
**模型配置替代版本（ModelConfigurationReplacement）**：
现有连接的配置草稿验证后追加的新连接；旧连接与秘密不覆盖，历史任务不漂移。
被替代版本不进入新任务目录，旧默认偏好需用户重选；详见 ADR-0043。
_Avoid_: 原地换 Key、自动改写历史任务、验证即保存

**模型使用偏好（ModelUsagePreference）**：
用户为新任务选择的默认模型连接及其默认连接模型；偏好可以改变，但不能改写既有
任务修订冻结的运行时分配。
管理员的本地兼容模型偏好仅引用当前已配置的模型名，不冒充已验证连接；读取和保存均复核角色。
明确选型优先于默认偏好。偏好失效保留原身份并要求重选，不静默切换；清除偏好只影响新任务。
...
**任务前对话草稿（Draft Conversation）**：
尚无来源和 TaskRevision 时的有界需求讨论。普通讨论只调用所选模型，不读取其他任务或资料；
用户明确要求采集在线数据时，语义判断交给既有采集分析执行链路，不能因没有附件而拒绝执行。
发送前明示所选模型与本次处理的数据范围，执行继续通过既有授权检查。对话暂存于当前浏览器的 Owner 草稿，
不等同于服务端不可变原始回合。添加资料并发送后，将本次原话及用于理解指代的助手回复
作为任务说明交给既有创建链路；助手回复不授予权限，也不是来源事实或正式交付。
_Avoid_: 草稿即已运行任务、正式交付、跨任务记忆、没有附件即不执行
