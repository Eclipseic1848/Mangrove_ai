...
export function ConnectorSourceIntake({ open, onOpenChange, ownerId, storageScope, purpose, onSelected }: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    ownerId: string;
    storageScope: string;
    purpose: string;
    onSelected: (snapshot: SourceSnapshot) => boolean;
}) {
    ...
    const [storageReady] = useState(() => { try {
    ...
    useEffect(() => { if (open && !frozen.current)
        setReadPurpose(purpose.trim() || "读取连接资料供当前任务使用"); }, [open]);
    useEffect(() => {
        if (!open || mode !== "database")
            return;
        let current = true;
        setCatalogError("");
        setCatalogLoading(true);
        void Promise.all([listOwnerConnections(), connectionId ? getConnectionSchema(connectionId) : Promise.resolve(null)])
            .then(([items, detail]) => { if (current) {
            setConnections(items);
            setSchema(detail);
        } })
            .catch(() => { if (current)
            setCatalogError("无法读取连接或表清单，请重新加载；没有发起数据读取。"); })
            .finally(() => { if (current)
    ...
    async function run(action: (identity: number) => Promise<void>) {
        if (inFlight.current || stopping.current)
            return;
        ...
        try {
            ...
            if (identity === request.current) {
                if (frozen.current)
                    setUnknown(true);
                setError(reason instanceof Error ? reason.message : "读取尚未确认");
            }
        }
        finally {
            if (identity === request.current) {
    ...
    function assertStored() {
        if (!storageReady)
            throw new Error("浏览器存储不可用，无法保存恢复身份；本次未发起新的读取。");
        if (localStorage.getItem(storageKey) !== stored.current) {
            setObsolete(true);
            setSnapshot(null);
            throw new Error("另一页面已更新此读取，请刷新页面查看最新请求；本页不会覆盖或加入旧资料。");
        }
    }
    ...
    function reset() {
        try {
            ...
            setError(reason instanceof Error ? reason.message : "恢复身份未能更新，请保留当前页面。");
            return false;
        }
        request.current++;
        frozen.current = null;
    ...
    function accept(response: ConnectorAttempt) {
        ...
        if (!original || response.idempotency_key !== original.key || !response.attempt_id || (original.attemptId && response.attempt_id !== original.attemptId) || !matches(response.allowed_scope)
            || (response.status === "succeeded" && (!response.snapshot || response.snapshot.source_kind !== "connector" || response.snapshot.attempt_id !== response.attempt_id || response.snapshot.snapshot_id !== response.snapshot_id || !response.snapshot.artifacts.length || !matches(response.snapshot.allowed_scope))))
            throw new Error("来源响应身份不一致，请保留原请求并核实");
        original.attemptId = response.attempt_id;
        persist();
        setAttempt(response);
        setUnknown(false);
        if (response.status === "succeeded" && response.snapshot)
            ...
            setError(ERROR_LABELS[response.error_code || ""] || "本次读取未形成可用资料，请核对来源后重新选择。");
        else if (response.status === "acquiring" && response.error_code !== "connector_checkpoint_ready") {
            setUnknown(true);
            setError("读取仍在进行或清理未确认，请查询原状态");
        }
    }
    async function read(identity: number, resume = false) {
        if (!scope || !readPurpose.trim() || readPurpose.length > 500 || (resume && unknown))
            return;
        ...
        try {
            response = await acquireConnectorSource({ ...frozen.current.payload, resume_checkpoint: resume }, frozen.current.key);
        }
        catch (reason) {
            ...
            if (identity === request.current && firstRequest && reason instanceof ApiError && reason.status === 409 && reason.message === "连接版本变化，请重新确认") {
                ...
                setScope(null);
                setUnknown(false);
                setError("连接版本变化，请重新核对范围；本次尚未开始读取。");
                return;
            }
            throw reason;
        }
    ...
    async function stop() {
        ...
        try {
            assertStored();
            const value = await cancelConnectorAcquisition(attempt.attempt_id);
            if (identity === request.current)
                accept(value);
        }
        catch (reason) {
            if (identity === request.current) {
                setUnknown(true);
                setError(reason instanceof Error ? reason.message : "停止结果尚未确认，请查询原状态；本次资料不能加入任务。");
            }
        }
        finally {
    ...
    {!scope && <button className="rounded-lg border px-3 py-2 text-sm" disabled={busy || (mode === "http_api" ? !url.trim() || !validPagination : catalogLoading || Boolean(catalogError) || !selectedTable?.primary_key.length)} onClick={() => void run(async (identity) => { const resolved = await resolveConnectorSource(selection()); if (identity === request.current)
        setScope(resolved); })}>{busy ? "正在核对范围…" : "核对读取范围"}</button>}
    {scope && <div className="space-y-2 text-sm"><p>{unknown ? "沿用上次核对版本，结果待确认" : "连接版本已锁定"}</p><ConnectorScopeFacts scope={scope}/><p className="text-xs text-muted-foreground">每段最多10批，总上限100原件或16 MiB；仅JSON/JSONL，超时10秒。</p>
      {unknown && <><p role="status">读取结果尚未确认；不会创建第二个请求。</p><button className="rounded-lg border px-3 py-2" disabled={busy || stopBusy} onClick={() => void run(async (identity) => { if (frozen.current?.attemptId) {
            const value = await getConnectorAcquisition(frozen.current.attemptId);
            if (identity === request.current)
                accept(value);
        }
        else
            await read(identity); })}>确认原读取结果</button></>}
      ...
      {attempt?.status === "cancelling" && <><p role="status">停止尚未确认</p><p>连接仍在收尾，本次资料不能加入任务。</p><button className="rounded-lg border px-3 py-2" disabled={busy || stopBusy} onClick={() => void run(async (identity) => { const value = await getConnectorAcquisition(attempt.attempt_id); if (identity === request.current)
            accept(value); })}>检查停止状态</button></>}
    ...
    {snapshot && <section aria-label="连接资料预览" className="space-y-3 text-sm"><p>已冻结 {snapshot.artifacts.length} 个原件；不保证整个数据库一致快照。</p>{snapshot.artifacts.map(item => <article key={item.artifact_id}><p>{item.title}</p><p className="break-all text-xs text-muted-foreground">{item.read_at} · SHA256 {item.content_sha256}</p><pre className="max-h-40 overflow-auto whitespace-pre-wrap rounded border p-2">{item.text_preview}</pre></article>)}<button className="rounded-lg bg-primary px-3 py-2 text-primary-foreground" onClick={() => { try {
        assertStored();
        if (onSelected(snapshot) && reset())
            onOpenChange(false);
    }
    catch (reason) {
        setError(reason instanceof Error ? reason.message : "资料尚未加入，请保留原请求。");
