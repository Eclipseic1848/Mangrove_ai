...
export function SourceReferenceFacts({ item }: { item: SourceReferencePage["items"][number] }) {
  return <>{item.task_id || "独立读取"} · {item.revision ? `V${item.revision}` : "版本未知"} · {({ revision: "保留版本", runtime: "任务运行", export: "导出", preview: "预览", delivery: "正式交付记录", producer: "生产归属" })[item.reference_kind]} · {({ retained: "已保留", active: "使用中", unknown: "结果未知", published: "已发布" })[item.state]}{item.delivery_id ? ` · 交付：${item.delivery_id}` : ""}{item.task_exists === false ? " · 原任务记录已清理" : item.in_recycle_bin ? " · 回收站中" : ""}</>;
}
