  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-muted/20">
      ...
      <div ref={contentRef} tabIndex={0} aria-label="来源内容" className="min-h-0 flex-1 overflow-auto p-4 focus-visible:ring-2 focus-visible:ring-ring"
        ...
        {task?.source_integrity?.deleted_source_keys.some(key => key === `web_artifact:${selectedUploadId}` || key === `connector_artifact:${selectedUploadId}` || key === `upload:${selectedUploadId}` || key === `delivery_output:${selectedUploadId}` || (source.data?.snapshot_id && key === `snapshot:${source.data.snapshot_id}`)) ? <p role="alert" className="text-sm text-destructive">来源已删除，原文不可再读取；不会从缓存恢复。独立正式结果仍可查看。</p> : selectedDerived ? <div className="space-y-3">
