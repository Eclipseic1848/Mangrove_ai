...
async function fixture(page: Page) {
  ...
  await page.route("**/*", async route => {
    ...
    if (new URL(route.request().url()).origin !== new URL(test.info().project.use.baseURL as string).origin) return route.abort();
    if (route.request().isNavigationRequest() && path === "/data-prep") return route.fulfill({ response: await route.fetch({ url: new URL("/e2e/fixtures/connector-source/app.html", test.info().project.use.baseURL as string).href }) });
    if (!path.startsWith("/api/")) return route.continue();
    if (path === "/api/data-sources/uploads/original-file") return route.fulfill({ json: { upload_id: "original-file", original_name: "原附件.csv", media_type: "text/csv", size_bytes: 20, sha256: "a".repeat(64) } });
    const body = route.request().headers()["content-type"]?.includes("application/json") ? route.request().postDataJSON() : null;
    if (route.request().method() !== "GET") writes.push({ path, body, key: route.request().headers()["idempotency-key"] });
    ...
    if (path === "/api/connector-sources/resolve") { scope.selection = body; return route.fulfill({ json: scope }); }
    if (path.endsWith("/acquisitions") && control.unknown) return route.abort();
    if (path.endsWith("/acquisitions") && control.pause && !body.resume_checkpoint) return route.fulfill({ status: 202, json: { attempt_id: "connector-attempt", idempotency_key: route.request().headers()["idempotency-key"], allowed_scope: scope, status: "acquiring", snapshot_id: null, snapshot: null, error_code: "connector_checkpoint_ready" } });
    if (path.endsWith("/acquisitions")) return route.fulfill({ status: 202, json: { attempt_id: "connector-attempt", idempotency_key: route.request().headers()["idempotency-key"], allowed_scope: scope, status: "succeeded", snapshot_id: snapshot.snapshot_id, snapshot, error_code: null } });
    if (path === "/api/semantic-workspace/context-options") return route.fulfill({ json: { templates: [], memories: [] } });
    if (path.endsWith("context-preview")) return route.fulfill({ json: { template: null, memories: [], proposed_changes: {}, preview_sha256: "sha256:" + "e".repeat(64) } });
    if (path === "/api/semantic-workspace/tasks") {
      if (route.request().method() === "GET") return route.fulfill({ json: control.created ? [control.task] : [] });
      control.created = true; return route.fulfill({ json: control.task ?? { task_id: "created-connector-task" } });
    }
    if (path === "/api/semantic-workspace/tasks/created-connector-task") return route.fulfill({ json: control.task });
    if (path.endsWith("/turns")) return route.fulfill({ json: { turns: [], results: [], proposals: [] } });
...
for (const cloud of [false, true]) test(`J1 ${cloud ? "云连接" : "本地"} 真实工作台保留附件，连接原件进入冻结任务与正式结果预览`, async ({ page }) => {
  ...
  await page.getByRole("button", { name: "核对读取范围", exact: true }).click(); await page.getByRole("button", { name: "读取并冻结资料", exact: true }).click();
  await page.getByRole("button", { name: "加入当前任务", exact: true }).click();
  if (cloud) await page.getByRole("combobox", { name: "模型连接", exact: true }).selectOption("cloud");
  ...
  if (cloud) expect(writes.find(item => item.path === "/api/semantic-workspace/tasks").body).toMatchObject({ model_connection_id: "cloud", external_api_confirmed: true });
...
test("R1 历史数据库资料显示原表字段并预览复用，不重新联网", async ({ page }) => {
  ...
  await page.goto("/e2e/fixtures/connector-source/index.html"); await page.getByRole("button", { name: "历史资料", exact: true }).click();
...
test("I3 另一页替换读取身份，旧响应不能覆盖存储或加入原件", async ({ page }) => {
  ...
  let release!: () => void; const pending = new Promise<void>(resolve => { release = resolve; }); let entered = false;
  ...
  await page.goto("/e2e/fixtures/connector-source/index.html"); await page.getByRole("button", { name: "从已有连接读取", exact: true }).click(); await page.getByLabel("公开数据地址", { exact: true }).fill(scope.selection.url);
  await page.getByRole("button", { name: "核对读取范围", exact: true }).click(); await page.getByRole("button", { name: "读取并冻结资料", exact: true }).click(); await expect.poll(() => entered).toBe(true);
...
test("I4 主任务草稿被另一页更新时关连接面板，保留原读取待确认", async ({ page }) => {
  ...
  await page.reload(); await page.getByRole("button", { name: "从已有连接读取", exact: true }).click();
  await expect(page.getByRole("button", { name: "确认原读取结果", exact: true })).toBeVisible();
});

for (const [width, theme] of [[390, "light"], [390, "dark"], [720, "light"], [1440, "dark"]] as const) test(`V1 ${width} ${theme} 连接弹窗键盘输入法与可见范围`, async ({ page }, info) => {
  await fixture(page); await page.setViewportSize({ width, height: width === 720 ? 450 : 900 }); await page.goto("/e2e/fixtures/connector-source/index.html");
  await page.evaluate(theme => document.documentElement.classList.toggle("dark", theme === "dark"), theme);
  const opener = page.getByRole("button", { name: "从已有连接读取", exact: true }); await opener.click();
  const dialog = page.getByRole("dialog", { name: "从已有连接读取", exact: true });
  const url = dialog.getByRole("textbox", { name: "公开数据地址", exact: true }); await url.fill("https://records.example.invalid/items");
  await url.dispatchEvent("keydown", { key: "Escape", isComposing: true }); await expect(dialog).toBeVisible();
  await dialog.getByRole("combobox", { name: "资料来源", exact: true }).focus(); await page.keyboard.press("Shift+Tab");
  ...
  await opener.click(); await expect(url).toHaveValue("https://records.example.invalid/items");
});

test("H1 公开HTTP来源先核范围再读取预览，显式加入完整任务", async ({ page }) => {
  const { writes, scope } = await fixture(page);
  await page.goto("/e2e/fixtures/connector-source/index.html"); await page.getByRole("button", { name: "从已有连接读取", exact: true }).click();
  await page.getByLabel("公开数据地址", { exact: true }).fill(scope.selection.url);
  ...
  expect(acquisition.body.expected_connection_version).toBe("b".repeat(64)); expect(acquisition.key).toBeTruthy();
...
test("H6 页码读取先明确上限，长任务要求不能变成无法恢复的422", async ({ page }) => {
  const { writes, scope } = await fixture(page);
  await page.goto("/e2e/fixtures/connector-source/index.html"); await page.locator("textarea").first().fill("长".repeat(501));
  await page.getByRole("button", { name: "从已有连接读取", exact: true }).click();
  await page.getByLabel("公开数据地址", { exact: true }).fill(scope.selection.url);
...
test("H2 分段暂停只显式继续原请求，关闭重开不重复读取", async ({ page }) => {
  const { writes, scope, control } = await fixture(page); control.pause = true;
  await page.goto("/e2e/fixtures/connector-source/index.html"); await page.getByRole("button", { name: "从已有连接读取", exact: true }).click();
  await page.getByLabel("公开数据地址", { exact: true }).fill(scope.selection.url);
  await page.getByRole("button", { name: "核对读取范围", exact: true }).click();
  await page.getByRole("button", { name: "读取并冻结资料", exact: true }).click();
  ...
  expect(reads).toHaveLength(2); expect(reads[1].key).toBe(reads[0].key);
  expect(reads[1].body).toEqual({ ...reads[0].body, resume_checkpoint: true });
});

test("H3 首次响应丢失后刷新保留原key和选择，明确确认才重放", async ({ page }) => {
  const { writes, scope, control } = await fixture(page); control.unknown = true;
  await page.goto("/e2e/fixtures/connector-source/index.html"); await page.getByRole("button", { name: "从已有连接读取", exact: true }).click();
  await page.getByLabel("公开数据地址", { exact: true }).fill(scope.selection.url);
  await page.getByRole("button", { name: "核对读取范围", exact: true }).click();
  await page.getByRole("button", { name: "读取并冻结资料", exact: true }).click();
  ...
  expect(writes.filter(item => item.path.endsWith("acquisitions"))).toHaveLength(1);
  control.unknown = false; await page.getByRole("button", { name: "确认原读取结果", exact: true }).click();
  await expect(page.getByRole("button", { name: "加入当前任务", exact: true })).toBeVisible();
  const reads = writes.filter(item => item.path.endsWith("acquisitions"));
...
test("H4 停止未知沿真实GET路由确认，不能提前冒称已取消", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "加入当前任务", exact: true })).toHaveCount(0);
  confirmed = true; await page.getByRole("button", { name: "检查停止状态", exact: true }).click();
  await expect(page.getByText("已停止读取", { exact: true })).toBeVisible();
});

test("H8 停止网络未知不能继续旧断点，只查原状态后才恢复继续", async ({ page }) => {
  const { scope, control, writes } = await fixture(page); control.pause = true;
  let queries = 0;
  ...
  expect(queries).toBe(1); await expect(page.getByRole("button", { name: "继续本次读取", exact: true })).toBeEnabled();
  await page.getByRole("button", { name: "继续本次读取", exact: true }).click();
  const reads = writes.filter(item => item.path.endsWith("acquisitions")); expect(reads).toHaveLength(2); expect(reads[1].body.resume_checkpoint).toBe(true); expect(reads[1].key).toBe(reads[0].key);
});

test("H5 继续在途仍可停止，迟到成功不能覆盖已确认停止", async ({ page }) => {
  const { scope, snapshot, control } = await fixture(page); control.pause = true;
  let release!: () => void; const pending = new Promise<void>(resolve => { release = resolve; });
  let key = ""; let resumed = false; let cancels = 0;
  await page.route("**/api/connector-sources/acquisitions", async route => {
    key = route.request().headers()["idempotency-key"];
    if (!route.request().postDataJSON().resume_checkpoint) return route.fallback();
    resumed = true; await pending;
  ...
  await page.route("**/api/semantic-workspace/source-acquisitions/connector-attempt/cancel", route => {
    cancels++; return route.fulfill({ json: { attempt_id: "connector-attempt", idempotency_key: key, allowed_scope: scope, status: "canceled", snapshot: null } });
  });
  await page.goto("/e2e/fixtures/connector-source/index.html"); await page.getByRole("button", { name: "从已有连接读取", exact: true }).click();
  await page.getByLabel("公开数据地址", { exact: true }).fill(scope.selection.url);
  await page.getByRole("button", { name: "核对读取范围", exact: true }).click(); await page.getByRole("button", { name: "读取并冻结资料", exact: true }).click();
  await page.getByRole("button", { name: "继续本次读取", exact: true }).click(); await expect.poll(() => resumed).toBe(true);
  try {
...
test("D1 复用本人数据库连接和表字段筛选，不创建凭据或SQL", async ({ page }) => {
  ...
  await page.getByRole("combobox", { name: "筛选字段", exact: true }).selectOption("region"); await page.getByLabel("等于这个值", { exact: true }).fill("华东");
  await page.getByRole("button", { name: "核对读取范围", exact: true }).click();
...
for (const [code, message] of [["authorization_expired", "来源授权已过期"], ["no_results", "本次范围内没有记录"], ["local_parse", "本地无法解析来源内容"], ["source_network", "来源网络或超时失败"]]) test(`E1 ${code}明确分类且可重新选择范围`, async ({ page }) => {
  const { writes, scope } = await fixture(page);
  ...
  await page.goto("/e2e/fixtures/connector-source/index.html"); await page.getByRole("button", { name: "从已有连接读取", exact: true }).click();
  await page.getByLabel("公开数据地址", { exact: true }).fill(scope.selection.url); await page.getByRole("button", { name: "核对读取范围", exact: true }).click(); await page.getByRole("button", { name: "读取并冻结资料", exact: true }).click();
...
test("I2 Owner切换后迟到原件不能回填新Owner或持久化正文", async ({ page }) => {
  const { scope, snapshot, writes } = await fixture(page);
  let release!: () => void; const pending = new Promise<void>(resolve => { release = resolve; }); let reading = false;
  ...
  await page.getByRole("button", { name: "返回原任务", exact: true }).click();
  await page.getByRole("button", { name: "切换合成Owner", exact: true }).click(); release();
  await page.getByRole("button", { name: "从已有连接读取", exact: true }).click();
