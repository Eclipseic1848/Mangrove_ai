  ...
  server.registerTool(
    ...
    async (params): Promise<CallToolResult> => {
      ...
      try {
        ...
        const needsBrowser = tool.annotations.requiresBrowser !== false;
        const context = needsBrowser
          ? await getContext()
        ...
        const {content, structuredContent} = needsBrowser
          ? await response.handle(tool.name, context)
          : await response.handleStorageOnly(tool.name);
